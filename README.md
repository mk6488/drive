...
A `functions/` workspace exists for future trusted role claim assignment only. It is not deployed, does not set Firebase custom claims, and does not create a live role assignment endpoint. The local dry run validates fake sample claim assignments and now prints audit-style planning output plus a future apply safety gate section only; it does not write audit records or real claims. Step 39 adds a trusted claims live apply runbook only; no live claim setting tool exists yet. The client app still cannot assign roles, promote users, or decide coach access by itself.
...
Step 39 adds `docs/46-trusted-claims-live-apply-runbook.md` and a short `functions/CLAIMS_RUNBOOK.md` pointer for future trusted claim apply planning. No live claim setting tool exists yet; no custom claims are set, Firebase Admin is not initialised, service account files are not needed, and live claim setting still requires a later explicit approved step.
