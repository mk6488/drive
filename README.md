...
Step 48 adds fake Firestore seed definitions and a dry-run plan for `example-club`, `example-j15-squad`, and `example-athlete` only. No Firestore seed data has been written, Firebase Admin is not initialised by `dryrun:seed`, and the repository provider remains mock backed.

Step 49 adds a guarded Firestore seed apply script foundation in the functions workspace only. `apply:seed` requires an explicit local JSON input path, the exact confirmation phrase, `requestedApplyMode: "live"`, `DRIVE_FIRESTORE_SEED_APPLY=true`, valid fake example seed data, and a passing safety gate before future writes can be reached. The command was not run in this step and no Firestore data has been written.
