...
A `functions/` workspace exists for future trusted role claim assignment only. It is not deployed, does not set Firebase custom claims, and does not create a live role assignment endpoint. The client app still cannot assign roles, promote users, or decide coach access by itself.
