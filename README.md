...
Mock mode checks the mock preview fixture by default. Firebase mode checks only the fake seed fixture when it is
intentionally enabled with env configuration. The route does not switch provider mode, write or delete Firestore data,
upload to Storage, or prove product workflows are ready.
...
Step 51 adds `/dev/repository-read-smoke-test` as a developer-only manual read preview. It checks the mock preview fixture in mock mode and the fake seed fixture in Firebase mode through the active repository provider only, does not import Firebase in screens or components, does not write or delete Firestore data, does not upload to Storage, does not prove product workflows are ready, and keeps mock mode as the default.
