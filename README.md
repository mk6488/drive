...
The app still defaults to mock mode, and Firebase mode should only be enabled intentionally with env configuration.
...
Step 51 adds `/dev/repository-read-smoke-test` as a developer-only manual read preview. It checks expected fake records through the active repository provider only, does not import Firebase in screens or components, does not write or delete Firestore data, and keeps mock mode as the default.
