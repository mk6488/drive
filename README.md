...
Step 48 adds fake Firestore seed definitions and a dry-run plan for `example-club`, `example-j15-squad`, and `example-athlete` only. No Firestore seed data has been written, no live seed script exists, Firebase Admin is not initialised by `dryrun:seed`, and the repository provider remains mock backed.
